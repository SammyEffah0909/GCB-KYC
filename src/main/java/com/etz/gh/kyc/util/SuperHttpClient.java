...
public class SuperHttpClient {
    ...
    //default timeout 30secs
    public static Map<String, String> doPost(String url, String payload, String token) {
        return doPost(url, payload, CONNECT_TIMEOUT, SOCKET_TIMEOUT, token);
    }
    ...
    public static Map<String, String> doPost(String url, String payload, int connectTimeout, int socketTimeout, String token) {
        CloseableHttpClient httpClient = HttpClients.createDefault();
        RequestConfig config = RequestConfig.custom().setConnectTimeout(connectTimeout * 1000).setSocketTimeout(socketTimeout * 1000).build();
        HttpPost httpPost = new HttpPost(url);
        httpPost.setConfig(config);
